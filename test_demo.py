...
def main(config):
    if config.mode == 'train':
        ...
        if not os.path.exists("%s/run-%s" % (config.save_fold, run)):
            ...
            os.mkdir("%s/run-%s/logs_depth" % (config.save_fold, run))
            os.mkdir("%s/run-%s/models_depth" % (config.save_fold, run))
